"""Experiment 66: roll-ratio features + structural diversity (exp51 config).

Key findings:
  - exp51 (no roll-ratio, 154 feats, 3 groups cs=0.8/0.5/0.7): local=0.2402, PUBLIC=0.2502 (BEST)
  - exp61 (roll-ratio, 158 feats, pure cs=0.5): local=0.2400, PUBLIC=0.2501 (2nd best)
  - exp57 (no roll-ratio, 154 feats, pure cs=0.5, leaves=511+255): local=0.2396, public=?

Hypothesis: roll-ratio features improve GENERALIZATION (+public LB even when local val dips).
Combining them with exp51's structural diversity (which was best on public) should give
the best of both worlds: diversity + generalization features.

Config = exp51 config + roll-ratio features:
  Group A (4 seeds): cs=0.8, ss=0.8
  Group B (3 seeds): cs=0.5, ss=0.7
  Group C (3 seeds): cs=0.7, ss=0.9
  All: leaves=511, alpha=0.55, 21d, 158 features
"""
...
print("  exp51 config + roll-ratio features (158 feats)")
print("  GroupA(cs=0.8) + GroupB(cs=0.5) + GroupC(cs=0.7)")
print("=" * 60)
sys.stdout.flush()
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    ...
    "note": (f"exp51-config + roll-ratio feats. GroupA(cs=0.8)+GroupB(cs=0.5)+GroupC(cs=0.7). "
...
print(f"  Raw:        WAPE={w_raw:.4f}  RBias={r_raw:.4f}  Total={t_raw:.4f}")
print(f"  Calibrated: WAPE={w_cal:.4f}  RBias={r_cal:.4f}  Total={t_cal:.4f}")
print(f"  vs exp51: 0.240207 local / 0.2502 public")
print("=" * 60)
