"""Experiment 37: Optimal alpha for near-zero raw RBias.

Key insight from alpha sweep:
  - alpha=0.52: individual RBias=-0.041 (under-predict 4.1%)
  - alpha=0.53: individual RBias=-0.034 (under-predict 3.4%)
  - alpha=0.55: individual RBias=+0.019 (over-predict 1.9%)
  - Zero crossing: alpha ~= 0.543

If raw RBias ~ 0, per-step calibration factors ~ 1.0,
i.e. calibration barely changes predictions, and:
  total ~ raw_WAPE ~ 0.237x (vs current best calibrated 0.2398)

10 seeds x alpha=0.543, leaves=127, 154 features.
"""
...
EXPERIMENT_NAME = "exp37_optimal_alpha"
...
print(f"  10 LGB seeds | alpha={ALPHA} (zero-RBias sweet spot)")
print(f"  154 features | per-step calib")
print(f"  Hypothesis: raw RBias ~ 0 -> total ~ raw WAPE ~ 0.237x")
print("=" * 60)
sys.stdout.flush()
...
for seed in SEEDS:
    ...
    joblib.dump(models, models_dir / f"lgb_a{int(ALPHA*1000)}_seed{seed}.pkl")
...
experiments.append({
    ...
    "params": {
        "seeds": SEEDS, "alpha": ALPHA, "num_leaves": 127,
        "n_models": len(SEEDS),
    ...
    "total": round(t_cal, 6),
    "note": (
        f"Zero-RBias alpha={ALPHA}. "
        f"Raw: WAPE={w_raw:.4f} RBias={r_raw:.4f} Total={t_raw:.4f}. "
        f"Cal: WAPE={w_cal:.4f} RBias={r_cal:.4f} Total={t_cal:.4f}. "
