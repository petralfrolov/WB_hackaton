"""Experiment 67: roll-ratio features + capacity diversity (exp57 config).

exp57 config (best local val=0.239609): leaves=511+255, cs=0.5, ss=0.7.
exp61 (roll-ratio, best public=0.2501): pure cs=0.5, 158 feats.

Hypothesis: combining best local config (cap diversity) with best-generalizing
features (roll-ratio) is additive — exp57+roll-ratio should beat both exp57 and exp61.

Config = exp57 capacity diversity + roll-ratio features:
  Group A (5 seeds): leaves=511, cs=0.5, ss=0.7
  Group B (5 seeds): leaves=255, cs=0.5, ss=0.7
  All: alpha=0.55, 21d, 158 features
"""
...
EXPERIMENT_NAME = "exp67_roll_ratio_cap_diversity"
ALPHA = 0.55
GROUPS = [
    ("L511_cs05", [42, 123, 456, 789, 1234],  511, 0.5, 0.7),
    ("L255_cs05", [2024, 7, 314, 99, 888],    255, 0.5, 0.7),
]
BASE_LGB = dict(n_estimators=5000, learning_rate=0.05, min_child_samples=10,
    subsample_freq=1, reg_alpha=0.1, reg_lambda=1.0, n_jobs=-1, verbose=-1)

print("=" * 60)
print(f"EXPERIMENT: {EXPERIMENT_NAME}")
print("  exp57 config (leaves=511+255, cs=0.5) + roll-ratio features (158)")
print("=" * 60)
sys.stdout.flush()
...
for label, seeds, leaves, cs, ss in GROUPS:
    print(f"\n>>> {label}: leaves={leaves} cs={cs} ss={ss} <<<")
    sys.stdout.flush()
    for seed in seeds:
        t0 = datetime.now()
        print(f"\n--- {label} seed={seed} [{t0.strftime('%H:%M:%S')}] ---")
        sys.stdout.flush()
        params = {**BASE_LGB, "num_leaves": leaves, "objective": "quantile", "alpha": ALPHA,
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    "params": {"groups": [(g[0],g[1],g[2],g[3],g[4]) for g in GROUPS], "alpha": ALPHA,
               "n_models": n_total, "train_days": 21,
               "features": "158 (154+4 roll-ratio)", "calibration": "per_step"},
    "wape": round(w_cal,6), "rbias": round(r_cal,6), "total": round(t_cal,6),
    "note": (f"exp57-config(leaves=511+255,cs=0.5) + roll-ratio feats. "
...
print(f"  Raw:        WAPE={w_raw:.4f}  RBias={r_raw:.4f}  Total={t_raw:.4f}")
print(f"  Calibrated: WAPE={w_cal:.4f}  RBias={r_cal:.4f}  Total={t_cal:.4f}")
print(f"  vs exp57: 0.239609 local; exp61: 0.2400 local / 0.2501 public")
print("=" * 60)
