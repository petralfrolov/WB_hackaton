"""Experiment 52: Feature + regularization diversity.

Building on exp51 — if structural diversity helps, push it further:
  - One group with very aggressive feature dropout (cs=0.4)
  - One group with strong L2 regularization (reg_lambda=5.0)
  - One group standard (cs=0.8 ss=0.8) as anchor

All 21d, leaves=511, alpha=0.55 — consistent with best config.

Groups (10 models):
  Group A (4 seeds): cs=0.8, ss=0.8, reg_lambda=1.0  (standard anchor)
  Group B (3 seeds): cs=0.4, ss=0.8, reg_lambda=1.0  (aggressive col dropout)
  Group C (3 seeds): cs=0.8, ss=0.8, reg_lambda=5.0  (strong L2)
"""
...
EXPERIMENT_NAME = "exp52_feature_reg_diversity"
ALPHA = 0.55

# (label, seeds, colsample_bytree, subsample, reg_lambda)
GROUPS = [
    ("A_std",    [42, 123, 456, 789],  0.8, 0.8, 1.0),
    ("B_cs04",   [1234, 2024, 7],      0.4, 0.8, 1.0),
    ("C_l2_5",   [314, 99, 888],       0.8, 0.8, 5.0),
]
...
print("  GroupA: cs=0.8 l2=1.0 | GroupB: cs=0.4 l2=1.0 | GroupC: cs=0.8 l2=5.0")
print("  All 21d, leaves=511, alpha=0.55")
print("=" * 60)
sys.stdout.flush()
...
# ── Train ─────────────────────────────────────────────────────────────────
for label, seeds, colsample, subsample, reg_lambda in GROUPS:
    print(f"\n>>> GROUP {label}: {len(seeds)} seeds | cs={colsample} ss={subsample} l2={reg_lambda} <<<")
    sys.stdout.flush()
    for seed in seeds:
        t0 = datetime.now()
...
experiments.append({
    ...
    "params": {
        ...
        "alpha": ALPHA, "num_leaves": 511, "min_child_samples": 10,
    ...
    "total": round(t_cal, 6),
    "note": (
        f"Feature+reg diversity: A(cs=0.8,l2=1), B(cs=0.4,l2=1), C(cs=0.8,l2=5). "
        f"All 21d leaves=511 alpha={ALPHA}. Raw: {t_raw:.4f}. Cal: {t_cal:.4f}."
    ),
})
