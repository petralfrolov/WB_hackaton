"""Experiment 50: Skewed multi-window — 3x21d + 7x42d.

Rationale:
  exp49 (5x21d + 5x42d TRUE windows) -> val=0.2407  (submitted, public pending)

If 42d models are individually better on the public/OOD test (more history),
putting more weight on them may improve further.

Config:
  - 3 models: 21-day window, alpha=0.55  [seeds 42, 123, 456]
  - 7 models: 42-day window, alpha=0.55  [seeds 789, 1234, 2024, 7, 314, 99, 888]
...
EXPERIMENT_NAME = "exp50_skewed_window"
SEEDS_21 = [42, 123, 456]
SEEDS_42 = [789, 1234, 2024, 7, 314, 99, 888]
ALPHA = 0.55
...
print(f"  3x21d + 7x42d | leaves=511 | alpha={ALPHA}")
print("  More weight on longer window — testing if 42d is better for OOD")
print("=" * 60)
sys.stdout.flush()
...
print("Preparing 42-day split...")
sys.stdout.flush()
X_fit_42, y_fit_42, X_valid_42, y_valid_42, X_test_42, _, _ = split_data(
    train_df_feat, feature_cols, train_days=42
)
X_fit_42, X_valid_42, X_test_42, _ = encode_categoricals(
    X_fit_42, X_valid_42, X_test_42, feature_cols
)
print(f"  42d: Fit={len(X_fit_42):,}  Valid={len(X_valid_42):,}")
sys.stdout.flush()

# ── Group 1: 3 seeds x 21d ────────────────────────────────────────────────
print(f"\n>>> GROUP 1: {len(SEEDS_21)} seeds x 21d <<<")
sys.stdout.flush()
vp_21, tp_21 = [], []
...
ens_21_raw = pd.DataFrame(
    np.mean([p.values for p in vp_21], axis=0),
    columns=FUTURE_TARGET_COLS, index=X_valid_21.index,
).clip(lower=0)
w_21r, r_21r, t_21r = metric.calculate_components(y_valid_21, ens_21_raw)
print(f"[21d {len(SEEDS_21)}-seed raw] WAPE={w_21r:.4f} RBias={r_21r:.4f} Total={t_21r:.4f}")
sys.stdout.flush()

# ── Group 2: 7 seeds x 42d ────────────────────────────────────────────────
print(f"\n>>> GROUP 2: {len(SEEDS_42)} seeds x 42d <<<")
sys.stdout.flush()
tp_42 = []
for seed in SEEDS_42:
    ...
    vp_42_tmp = predict_steps(models, X_valid_42)
    w, r, t = metric.calculate_components(y_valid_42, vp_42_tmp)
    elapsed = (datetime.now() - t0).seconds
    print(f"  seed={seed}: WAPE={w:.4f} RBias={r:.4f} Total={t:.4f}  [{elapsed}s]")
    sys.stdout.flush()
    tp_42.append(tp)
...
all_tp = tp_21 + tp_42
n_total = len(all_tp)
print(f"\n[Ensemble] Blending {n_total} predictions ({len(SEEDS_21)}x21d + {len(SEEDS_42)}x42d)...")
sys.stdout.flush()

test_ens = pd.DataFrame(
    np.mean([tp.values for tp in all_tp], axis=0),
    columns=FUTURE_TARGET_COLS,
    index=X_test_21.index,
).clip(lower=0)

# ── Calibrate from 21d val ────────────────────────────────────────────────
factors = {}
ens_21_cal = ens_21_raw.copy()
for col in FUTURE_TARGET_COLS:
    f = float(y_valid_21[col].sum()) / max(float(ens_21_raw[col].sum()), 1e-9)
    ens_21_cal[col] = (ens_21_raw[col] * f).clip(lower=0)
...
print(f"[21d {len(SEEDS_21)}-model cal] WAPE={w_cal:.4f} RBias={r_cal:.4f} Total={t_cal:.4f}")
...
experiments.append({
    ...
    "total": round(t_cal, 6),
    "note": (
        f"Skewed multi-window: {len(SEEDS_21)}x21d + {len(SEEDS_42)}x42d, leaves=511 alpha={ALPHA}. "
        f"21d {len(SEEDS_21)}-model raw: {t_21r:.4f}. Cal: {t_cal:.4f}. "
...
print(f"  Test: {n_total} models (3x21d + 7x42d), calibrated from 21d val")
print("=" * 60)
