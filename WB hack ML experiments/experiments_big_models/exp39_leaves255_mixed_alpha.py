"""Experiment 39: num_leaves=255 + mixed alpha 0.52+0.55.

Key insight: exp35 (leaves=255, alpha=0.55) scored 0.2515 on public LB
vs exp_best_features_full (leaves=127) which scored 0.253.
This suggests higher capacity models generalise better to test distribution.

Hypothesis: combine benefits of:
  - num_leaves=255 (more capacity, exp35 signal)
  - mixed alpha 0.52+0.55 (exp_best architecture, best val result so far)

Config:
  - 5 seeds x alpha=0.52 + 5 seeds x alpha=0.55 = 10 models
  - num_leaves=255 for all
...
EXPERIMENT_NAME = "exp39_leaves255_mixed_alpha"
SEEDS = [42, 123, 456, 789, 1234]
ALPHAS = [0.52, 0.55]  # 5 seeds each = 10 models

BASE_LGB = dict(
    n_estimators=5000, learning_rate=0.05, num_leaves=255,
...
print(f"  10 LGB: 5x alpha=0.52 + 5x alpha=0.55 | num_leaves=255")
print(f"  154 features | per-step calib")
print(f"  Hypothesis: leaves=255 public boost + mixed alpha diversity")
print("=" * 60)
sys.stdout.flush()
...
# -- Train all models ---------------------------------------------------------
for alpha in ALPHAS:
    print(f"\n=== alpha={alpha} ({len(SEEDS)} seeds, leaves=255) ===")
    sys.stdout.flush()
    for seed in SEEDS:
        t0 = datetime.now()
        print(f"\n--- LGB alpha={alpha} seed={seed} [{t0.strftime('%H:%M:%S')}] ---")
        sys.stdout.flush()
        ...
        print(f"  seed={seed} a={alpha}: WAPE={w:.4f} RBias={r:.4f} Total={t:.4f}  [{elapsed}s]")
        sys.stdout.flush()
        valid_preds_list.append(vp)
        test_preds_list.append(tp)
        joblib.dump(models, models_dir / f"lgb_a{int(alpha*100):03d}_seed{seed}.pkl")
...
experiments.append({
    ...
    "params": {
        "seeds": SEEDS, "alphas": ALPHAS, "num_leaves": 255,
    ...
    "total": round(t_cal, 6),
    "note": (
        f"leaves=255, 5x a=0.52 + 5x a=0.55. "
