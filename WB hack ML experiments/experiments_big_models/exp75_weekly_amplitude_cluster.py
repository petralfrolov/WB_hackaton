"""Experiment 75: 3-cluster by weekly amplitude (peak vs flat demand patterns).

Routes with strong weekly periodicity (offices, B2B flows) follow very different
patterns from routes with flat 24/7 demand (e.g. food/e-commerce fulfillment).

Cluster by weekly_amplitude = (max_dow_avg - min_dow_avg) / (mean_target + 1e-6)
where dow_avg = mean target per day-of-week for a given route.

  Flat:     low weekly_amplitude (demand is roughly uniform across days)
  Moderate: medium amplitude
  Peaked:   high amplitude (strong weekly cycles)

A model trained on only peaked routes can pay more attention to weekday encoding.
A model trained on flat routes can rely more on intraday patterns.

3 clusters, 3 seeds each, cs=0.5, ss=0.7, leaves=511, alpha=0.55, 21d, 158 feats.
"""
...
EXPERIMENT_NAME = "exp75_weekly_amplitude_cluster"
SEEDS = [42, 123, 456]
ALPHA = 0.55
COLSAMPLE = 0.5
SUBSAMPLE = 0.7
...
print(f"  3 clusters by weekly amplitude (flat/moderate/peaked) x {len(SEEDS)} seeds each")
...
# ── Compute weekly amplitude clusters ─────────────────────────────────────────
tmp = train_df_raw.copy()
tmp["_dayofweek"] = pd.to_datetime(tmp["timestamp"]).dt.dayofweek  # 0=Mon..6=Sun
dow_avg = tmp.groupby(["route_id", "_dayofweek"])[TARGET_COL].mean().unstack(fill_value=0)
# weekly_amplitude = (max_dow - min_dow) / (global_mean + eps)
route_mean   = train_df_raw.groupby("route_id")[TARGET_COL].mean()
route_amp    = (dow_avg.max(axis=1) - dow_avg.min(axis=1)) / (route_mean + 1e-6)
del tmp

q33 = route_amp.quantile(1/3)
q67 = route_amp.quantile(2/3)
print(f"Weekly amplitude thresholds: p33={q33:.3f}  p67={q67:.3f}")

def assign_amp_cluster(route_id):
    a = route_amp.get(route_id, route_amp.mean())
    if a < q33:   return "flat"
    elif a < q67: return "moderate"
    else:          return "peaked"

route_cluster = {r: assign_amp_cluster(r) for r in route_amp.index}
clusters = ["flat", "moderate", "peaked"]
for c in clusters:
    n = sum(1 for v in route_cluster.values() if v == c)
    amps = route_amp[[r for r, cl in route_cluster.items() if cl == c]]
    means = route_mean[[r for r, cl in route_cluster.items() if cl == c]]
    print(f"  Cluster '{c}': {n} routes  amp=[{amps.min():.3f},{amps.max():.3f}]  "
          f"mean_target=[{means.min():.1f},{means.max():.1f}]")
sys.stdout.flush()
...
X_fit_g["_cluster"]   = X_fit_g["route_id"].map(route_cluster).fillna("moderate")
X_valid_g["_cluster"] = X_valid_g["route_id"].map(route_cluster).fillna("moderate")
X_test["_cluster"]    = X_test["route_id"].map(route_cluster).fillna("moderate")
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    "params": {"clusters": 3, "cluster_type": "weekly_amplitude",
               "amp_thresholds": {"p33": round(q33,4), "p67": round(q67,4)},
    ...
    "note": (f"3 clusters by weekly amplitude=(max_dow_avg-min_dow_avg)/mean (flat/moderate/peaked). "
             f"p33={q33:.4f} p67={q67:.4f}. Raw:{t_raw:.4f} Cal:{t_cal:.4f}.")})
