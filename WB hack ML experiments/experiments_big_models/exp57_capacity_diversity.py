"""Experiment 57: Capacity + colsample diversity.

exp51 (best=0.2502): cs/ss diversity works.
exp40 found leaves=511 > leaves=255.
Hypothesis: mixing leaves={511, 255} in same ensemble adds capacity
diversity on top of already-proven cs/ss diversity.

Groups (10 models):
  A (5 seeds): leaves=511, cs=0.5, ss=0.7 — pure GroupB config
  B (5 seeds): leaves=255, cs=0.5, ss=0.7 — half capacity same cs/ss
"""
import json, sys
...
EXPERIMENT_NAME = "exp57_capacity_diversity"
ALPHA = 0.55
GROUPS = [
...
print("  Capacity diversity: 5x(leaves=511) + 5x(leaves=255) | cs=0.5 ss=0.7")
print("=" * 60)
sys.stdout.flush()
...
BASE_LGB = dict(learning_rate=0.05, min_child_samples=10, subsample_freq=1,
    reg_alpha=0.1, reg_lambda=1.0, n_jobs=-1, verbose=-1)

for label, seeds, leaves, cs, ss in GROUPS:
    print(f"\n>>> {label}: {len(seeds)} seeds | leaves={leaves} cs={cs} ss={ss} <<<")
    sys.stdout.flush()
    for seed in seeds:
        t0 = datetime.now()
        print(f"\n--- {label} seed={seed} [{t0.strftime('%H:%M:%S')}] ---")
        sys.stdout.flush()
        params = {**BASE_LGB, "n_estimators": 5000, "num_leaves": leaves,
                  "objective": "quantile", "alpha": ALPHA, "random_state": seed,
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    "params": {"groups": [(g[0],g[1],g[2],g[3],g[4]) for g in GROUPS], "alpha": ALPHA,
               "n_models": n_total, "train_days": 21, "calibration": "per_step"},
    "wape": round(w_cal,6), "rbias": round(r_cal,6), "total": round(t_cal,6),
    "note": f"5x(leaves=511)+5x(leaves=255), cs=0.5,ss=0.7. Raw:{t_raw:.4f} Cal:{t_cal:.4f}"})
