"""Experiment 56: No standard anchor — 3 diverse low-cs groups.

exp51 had anchor GroupA (cs=0.8) which may be dragging ensemble toward
over-fitted territory. Try removing the standard group entirely.

Groups (10 models):
  B (4 seeds): cs=0.5, ss=0.7   — best from exp51
  C (3 seeds): cs=0.7, ss=0.9   — GroupC from exp51
  D (3 seeds): cs=0.35, ss=0.65 — new, more aggressive
No cs=0.8 group at all.
"""
import json, sys
...
EXPERIMENT_NAME = "exp56_no_anchor"
ALPHA = 0.55
GROUPS = [
    ("B_cs05",  [42, 123, 456, 789],  0.5,  0.7),
    ("C_cs07",  [1234, 2024, 7],      0.7,  0.9),
    ("D_cs035", [314, 99, 888],       0.35, 0.65),
]
...
print("  No cs=0.8 anchor | B(cs=0.5)+C(cs=0.7)+D(cs=0.35) | leaves=511")
print("=" * 60)
sys.stdout.flush()
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    ...
    "note": f"No anchor: B(cs=0.5)+C(cs=0.7)+D(cs=0.35). Raw:{t_raw:.4f} Cal:{t_cal:.4f}"})
