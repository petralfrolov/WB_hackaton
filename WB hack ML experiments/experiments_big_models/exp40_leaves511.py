"""Experiment 40: Very high capacity LGB — num_leaves=511.

Key finding: exp35 (leaves=255) scored 0.2515 on public LB vs 0.253 baseline.
Hypothesis: trend continues — more leaves -> better generalisation on public test.
Trying num_leaves=511: even deeper trees to capture complex feature interactions.
min_child_samples=10 (relaxed from 20) to allow finer splits.

Config:
  - 10 seeds x alpha=0.55 (matched exp35 config, just scale leaves)
...
EXPERIMENT_NAME = "exp40_leaves511"
...
print(f"  Hypothesis: more leaf capacity -> better public LB generalisation")
print("=" * 60)
sys.stdout.flush()
...
for seed in SEEDS:
    t0 = datetime.now()
    print(f"\n--- LGB leaves=511 alpha={ALPHA} seed={seed} [{t0.strftime('%H:%M:%S')}] ---")
    sys.stdout.flush()
...
experiments.append({
    ...
    "params": {
        ...
        "min_child_samples": 10, "n_models": len(SEEDS),
    ...
    "total": round(t_cal, 6),
    "note": (
        f"LGB leaves=511 min_child=10, 10 seeds alpha={ALPHA}. "
