"""Experiment 55: Graduated cs diversity — 4 groups spread across 0.3..0.8.

Instead of 3 groups, use 4 with wider spread including very low cs=0.3.
The idea: more extreme feature dropout may generalise better on OOD test.

Groups (10 models):
  A (3 seeds): cs=0.8, ss=0.8  — standard anchor
  B (3 seeds): cs=0.5, ss=0.7  — known best
  C (2 seeds): cs=0.35, ss=0.65 — aggressive
  D (2 seeds): cs=0.65, ss=0.85 — mild intermediate
"""
import json, sys
...
EXPERIMENT_NAME = "exp55_graduated_cs"
ALPHA = 0.55
GROUPS = [
    ("A_cs08", [42, 123, 456],   0.8,  0.8),
    ("B_cs05", [789, 1234, 2024], 0.5,  0.7),
    ("C_cs035",[7, 314],          0.35, 0.65),
    ("D_cs065",[99, 888],         0.65, 0.85),
]
...
print("  4 groups: cs=0.8/0.5/0.35/0.65 | graduated diversity | leaves=511")
print("=" * 60)
sys.stdout.flush()
...
exps.append({"timestamp": datetime.now().isoformat(timespec="seconds"), "name": EXPERIMENT_NAME,
    ...
    "note": f"4 graduated groups cs=0.8/0.5/0.35/0.65. Raw:{t_raw:.4f} Cal:{t_cal:.4f}"})
