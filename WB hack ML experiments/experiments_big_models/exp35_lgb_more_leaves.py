"""Experiment 35: LGB only, num_leaves=255 (more capacity), 154 features.

Hypothesis: with 154 features, the model may be underfitting with num_leaves=127.
Doubling leaves (255) should allow capturing more complex interactions.

Config:
  - 10 LGB seeds: [42,123,456,789,1234,2024,7,314,99,888]
  - alpha=0.55 (single, best from prior experiments)
  - num_leaves=255, rest same as baseline
  - 154 features (extended + rolling_cv + lag_ratios)
  - per-step bias calibration

Baseline: exp_best_features_full = 0.2405 (10 LGB, leaves=127, mixed alpha)
Target: beat 0.2398
"""
...
EXPERIMENT_NAME = "exp35_lgb_more_leaves"
...
print(f"  10 LGB seeds | num_leaves=255 | alpha={ALPHA}")
print(f"  154 features (extended+cv+lr) | per-step calib")
print("=" * 60)
sys.stdout.flush()
...
for seed in SEEDS:
    t0 = datetime.now()
    print(f"\n--- LGB alpha={ALPHA} num_leaves=255 seed={seed} [{t0.strftime('%H:%M:%S')}] ---")
    sys.stdout.flush()
...
experiments.append({
    ...
    "params": {
        "seeds": SEEDS, "alpha": ALPHA, "num_leaves": 255,
        "n_models": len(SEEDS), "features": "extended+rolling_cv+lag_ratios (154)",
        "calibration": "per_step",
    },
    "wape": round(w_cal, 6),
    "rbias": round(r_cal, 6),
    "total": round(t_cal, 6),
    "note": f"LGB num_leaves=255, 10 seeds alpha={ALPHA}. Raw={t_raw:.4f} -> Cal={t_cal:.4f}",
})
