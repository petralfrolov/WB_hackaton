"""Compute non-conformity scores on the held-out validation set.

Non-conformity score formula:
    score = |y_actual - y_hat|
...
Output: data/non_conformity_scores.csv  with columns: route_id, horizon, score
"""
...
OUT_PATH = Path("data/non_conformity_scores.csv")
...
# ── Compute non-conformity scores per horizon ─────────────────────────────────
...
print(result_df.groupby("horizon")["score"].describe().round(4).to_string())
