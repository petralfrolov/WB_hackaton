"""Experiment: Best features full ensemble.

Exp29 architecture (10 LGB, mixed alpha 0.52+0.55, per-step calibration)
plus 7 winning features from screening:
  - rolling_cv: target_cv_{4,8,48}
  - lag_ratios: lag_ratio_1_48, lag_ratio_1_336, momentum_1_ema8, momentum_1_ema24

Baseline exp29: Total=0.2379  |  exp28: Total=0.2402
Screening found cv+lr combo improves step-1 WAPE by -0.000175.
"""
...
def add_winning_features(df):
    ...
    # --- rolling_cv: coefficient of variation ---
    for w in [4, 8, 48]:
...
print(f"  +7 winning features (rolling_cv + lag_ratios)")
print("=" * 60)
sys.stdout.flush()
...
history.append({
    ...
    "params": dict(
        sample_frac=1.0, seeds=SEEDS, alphas=[0.52, 0.55],
        models="10lgb", calibration="per_step",
        extra_features="rolling_cv+lag_ratios (+7)",
    ),
    "wape": round(w_c, 6), "rbias": round(r_c, 6), "total": round(t_c, 6),
})
...
sys.stdout.flush()
# Reload original test_df (with id column) for submission building
_, test_df_orig = load_data()
sub = build_submission(test_corrected, X_test, inference_ts, test_df_orig, bias_factor=1.0)
...
print(f"  Validation Total = {t_c:.6f}")
print(f"  (Baseline exp28 = 0.2402, exp29 = 0.2379)")
print(f"{'='*60}")
