...
async def get_config(state: AppState = Depends(get_state)):
    ...
    return {**state.vehicles_cfg, "confidence_level": state.confidence_level, "granularity": state.granularity}


@router.post("/config")
async def set_config(payload: dict, state: AppState = Depends(get_state)):
...
async def update_settings(s: Settings, state: AppState = Depends(get_state)):
    ...
    # granularity is stored separately in AppState
    if "granularity" in payload:
        g = float(payload.pop("granularity"))
        if g in (0.5, 1.0, 2.0):
            state.granularity = g
            state.vehicles_cfg["granularity"] = state.granularity
...
async def get_route_distances(state: AppState = Depends(get_state)):
...
async def put_route_distances(items: List[RouteDistance], state: AppState = Depends(get_state)):
    ...
    atomic_write_json(_ROUTE_DISTANCES_PATH, {"route_distances": state.route_distances})
    return items
